...
void transcribe_with_whisper(const char* audio_file, const char* language) {
    char command[256];
    snprintf(command, sizeof(command), ".\\.venv\\Scripts\\python.exe whisper_transcribe.py %s %s", audio_file, language); // ใช้ Python ใน .venv
    printf("Starting Whisper transcription...\n");
    int result = system(command);
    if (result == -1) {
        fprintf(stderr, "Error executing Whisper transcription.\n");
    } else if (result != 0) {
        fprintf(stderr, "Whisper transcription exited with code %d.\n", result);
    } else {
